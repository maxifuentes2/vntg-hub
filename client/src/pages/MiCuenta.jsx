...
export default function MiCuenta() {
    ...
    const [editField, setEditField] = useState(null); // 'address', 'city', etc.
    ...
    const DataRow = ({ label, field, value, icon: Icon }) => (
        <div className="flex items-center justify-between p-4 bg-white dark:bg-[#1a1a1a] border border-zinc-200 dark:border-white/5 group">
            <div className="flex items-center gap-4 flex-1">
                <div className="w-8 h-8 flex items-center justify-center text-brand-orange/50">
                    <Icon size={18} />
                </div>
    ...
    return (
        <div className="bg-zinc-50 dark:bg-brand-dark min-h-screen pt-32 pb-20 px-4 font-sans">
            <div className="max-w-[800px] mx-auto space-y-12">
                
                {/* CUADRO DE DATOS (ARRIBA) */}
                <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-zinc-200 dark:bg-white/5 border border-zinc-200 dark:border-white/5 shadow-xl overflow-hidden rounded-xl">
                        ...
                        <div className="p-4 bg-zinc-100 dark:bg-[#111] border-b border-zinc-200 dark:border-white/5">
                            ...
                            <p className="text-sm font-bold italic dark:text-white uppercase">{user.name}</p>
                        </div>
                        <div className="p-4 bg-zinc-100 dark:bg-[#111] border-b border-zinc-200 dark:border-white/5">
                            <p className="text-[9px] font-black uppercase text-zinc-500">Email</p>
                            <p className="text-sm font-bold italic dark:text-white">{user.email}</p>
                        </div>
                        {/* Datos Editables */}
                ...
                {/* HISTORIAL DE COMPRAS (ABAJO) */}
                <section>
                    <h2 className="text-xs font-black uppercase italic tracking-[0.3em] text-zinc-500 mb-6 flex items-center gap-3">
                        <Package size={14} className="text-brand-orange" /> Mis Compras con Éxito
                    </h2>
                    {loading ? (
                        <div className="text-center py-10 animate-pulse font-black italic text-zinc-500">CARGANDO HISTORIAL...</div>
                    ) : (
                        <div className="space-y-3">
                            {orders.length > 0 ? orders.map(order => (
                                <div key={order.id} className="bg-white dark:bg-[#111] border border-zinc-200 dark:border-white/5 p-5 flex justify-between items-center hover:border-brand-orange/50 transition-all">
                                    <div>
                                        ...
                                        <p className="font-bold italic uppercase text-sm dark:text-white">Orden #{order.id.slice(0,8)}</p>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <p className="text-xl font-black italic mt-1 dark:text-white">${parseFloat(order.total).toLocaleString('es-AR')}</p>
                                    </div>
                                </div>
                            )) : (
