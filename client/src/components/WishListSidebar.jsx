                    ...
                    {wishListItems.map((item) => (
                        <div key={item.id} className="flex gap-4 bg-zinc-50 dark:bg-[#1a1a1a] p-3 rounded-2xl border border-zinc-200 dark:border-white/5 relative">
                            ...
                            <div className="flex flex-col items-end justify-between ml-2">
                                ...
                                <button onClick={(e) => handleMoveToCart(e, item)} disabled={item.stock === 0} className={`p-2 rounded-lg transition-colors ${item.stock === 0 ? 'bg-zinc-200 dark:bg-zinc-800 text-zinc-400 cursor-not-allowed' : 'bg-zinc-900 dark:bg-white text-white dark:text-[#111111] hover:bg-brand-orange shadow-md'}`}><ShoppingCart size={16} /></button>
                            </div>
                        </div>
                    ))}
                </div>
