...
export default function Navbar({ onOpenCart, onOpenWishList }) {
    ...
    useEffect(() => {
        ...
        // Verificar sesión del usuario con validación anti-crasheos
        ...
        if (storedUser && storedUser !== "undefined") {
            try {
                ...
                console.error("Dato corrupto en localStorage, limpiando...", error);
                localStorage.removeItem('vntg_user'); // Limpia si hay error
            }
        } else if (storedUser === "undefined") {
            // Si por error se guardó el string "undefined", lo borramos
                                ...
                                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#111111] border border-zinc-200 dark:border-white/5 shadow-2xl py-2 z-50">
                                    {user ? (
                                        <>
                                            ...
                                            {/* BOTÓN "OCULTO" QUE SOLO VE EL ADMIN */}
                                            {user.email === 'hubvntg@gmail.com' && (
                                                <Link 
                                                    to="/admin" 
                                                    onClick={() => setIsUserMenuOpen(false)}
                                                    className="flex items-center gap-3 px-4 py-3 text-xs font-black uppercase italic text-brand-orange hover:bg-brand-orange hover:text-white transition-colors"
                                                >
